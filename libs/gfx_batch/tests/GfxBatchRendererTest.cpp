...
const struct {
  ...
  Cr::Containers::Array<Cr::Containers::Triple<const char*, gfx_batch::RendererFileFlags, const char*>> gltfFilenames;
  ...
  {"generate mipmap", {Cr::InPlaceInit, {
    {"batch.gltf", gfx_batch::RendererFileFlag::GenerateMipmap, nullptr}}},
    1,
    {1, 1, 0, 1},
    0.0f, 0.0f},
  ...
  {"multiple textures, generate mipmap", {Cr::InPlaceInit, {
    {"batch-multiple-textures.gltf", gfx_batch::RendererFileFlag::GenerateMipmap, nullptr}}},
    /* Each has a separate texture */
    3,
    {4, 2, 0, 1},
    0.0f, 0.0f},
  ...
  {"multiple files, compressed textures, generate mipmap", {Cr::InPlaceInit, {
    {"batch-square-circle-triangle-compressed.gltf", gfx_batch::RendererFileFlag::GenerateMipmap, nullptr},
    {"batch-four-squares-compressed.gltf", gfx_batch::RendererFileFlag::GenerateMipmap, nullptr}}},
    2,
    {4, 2, 0, 1},
    1.5f, 0.5f},
  {"multiple files, some whole-file", {Cr::InPlaceInit, {
    {"batch-square-circle-triangle.gltf", {}, nullptr},
    {"batch-four-squares-deep-hierarchy-whole-file.gltf", gfx_batch::RendererFileFlag::Whole, "four squares"}}},
    /* Square, circle and triangle are a single mesh but the hierarchical four
       squares are a separate file */
    2,
    {4, 2, 0, 1},
...
const struct {
  ...
  {"multiple files with deep hierarchy as a whole", {Cr::InPlaceInit, {
    {"batch-square-circle-triangle.gltf",
      gfx_batch::RendererFileFlag::Whole, nullptr},
    {"batch-four-squares-deep-hierarchy-whole-file.gltf",
      gfx_batch::RendererFileFlag::Whole, "four squares"}}},
    {}, 5, 4, 4, 0xcc/255.0f,
    "GfxBatchRendererTestMeshHierarchy.png"},
  ...
  {"no textures", {Cr::InPlaceInit, {
    {"batch.gltf", {}, nullptr}}},
    gfx_batch::RendererFlag::NoTextures, 5, 4, 1, 1.0f,
    "GfxBatchRendererTestMeshHierarchyNoTextures.png"},
  {"multiple meshes, no textures", {Cr::InPlaceInit, {
    {"batch-multiple-meshes.gltf", {}, nullptr}}},
    gfx_batch::RendererFlag::NoTextures, 5, 4, 4, 1.0f,
    "GfxBatchRendererTestMeshHierarchyNoTextures.png"},
...
void GfxBatchRendererTest::defaults() {
  ...
  gfx_batch::RendererStandalone renderer{
      ...
      gfx_batch::RendererStandaloneConfiguration{}
          /* No QuietLog for the first instance, to verify at least once that
             the log *is* printed */
          .setFlags(testCaseInstanceId() == 0 ? gfx_batch::RendererStandaloneFlags{} : gfx_batch::RendererStandaloneFlag::QuietLog)
  };
  // clang-format on
  ...
  for (std::size_t i = 0; i != renderer.sceneCount(); ++i) {
    ...
    gfx_batch::SceneStats stats = renderer.sceneStats(i);
...
void GfxBatchRendererTest::singleMesh() {
  ...
  gfx_batch::RendererStandalone renderer{
      ...
      gfx_batch::RendererStandaloneConfiguration{}
          .setFlags(gfx_batch::RendererStandaloneFlag::QuietLog)
  };
  // clang-format on
  ...
  gfx_batch::SceneStats stats = renderer.sceneStats(0);
...
void GfxBatchRendererTest::multipleScenes() {
  ...
  gfx_batch::SceneStats stats0 = renderer.sceneStats(0);
  ...
  gfx_batch::SceneStats stats1 = renderer.sceneStats(1);
  ...
  gfx_batch::SceneStats stats2 = renderer.sceneStats(2);
  ...
  gfx_batch::SceneStats stats3 = renderer.sceneStats(3);
  ...
  if (data.sceneLightCount) {
    ...
    for (Mn::UnsignedInt i = 0; i != data.sceneLightCount; ++i) {
      CORRADE_COMPARE(renderer.addLight(
                          0, 6 + (i % 2),
                          i % 2 ? gfx_batch::RendererLightType::Directional
...
void GfxBatchRendererTest::clearLights() {
  ...
  CORRADE_COMPARE(
      renderer.addLight(0, 6, gfx_batch::RendererLightType::Point), 0);
  CORRADE_COMPARE(
      renderer.addLight(0, 7, gfx_batch::RendererLightType::Directional),
      1);
  ...
  renderer.addLight(0, 6, gfx_batch::RendererLightType::Point);
  renderer.addLight(0, 7, gfx_batch::RendererLightType::Directional);
  CORRADE_COMPARE(renderer.lightColors(0).size(), 2);
